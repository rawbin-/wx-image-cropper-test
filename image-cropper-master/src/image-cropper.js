Component({
  ...
  //     box-shadow: 0 0 100rpx 1000rpx rgba(0,0,0,0.8)
  //   }
  //     .border{
  //     display: none;
  //   }

  externalClasses: ['head-img-border', 'head-img-middle-content'],
  properties: {
    /**
     * 图片路径
     */
  ...
  data: {
    ...
    watch: {
      ...
      cut_top (value, that) {
        that._cutDetectionPosition()// todo
        if (that.data.limit_move) {
          !that.data._canvas_overflow && that._draw()
        }
      },
  ...
  methods: {
    ...
    getImg (getCallback) {
      this._draw(() => {
        this.pressImg(1, getCallback)
      })
    },

    pressImg (quality, getCallback) { // 压缩图片
      wx.canvasToTempFilePath({
        ...
        success: (res) => {
          if (wx.getFileSystemManager().readFileSync(res.tempFilePath, 'base64').length / 1024 > 600) {
            this.pressImg(quality - 0.2, getCallback)
          } else {
            getCallback({
    ...
    _cutDetectionPosition () {
      const _cutDetectionPositionTop = () => {
        ...
        if (this.data.info.windowHeight - this.data.height < null) {
          this.setData({ // todo
    ...
    _move (event) {
      ...
      if (event.touches.length == 1) {
        ...
        const top = (event.touches[0].clientY - this.data._touch_img_relative[0].y)
        // 图像边缘检测,防止截取到空白
        ...
        if (!this.data.disable_rotate) {
          const first_atan = 180 / Math.PI * Math.atan2(_touch_img_relative[0].y, _touch_img_relative[0].x)
          const first_atan_old = 180 / Math.PI * Math.atan2(this.data._touch_img_relative[0].y, this.data._touch_img_relative[0].x)
          const second_atan = 180 / Math.PI * Math.atan2(_touch_img_relative[1].y, _touch_img_relative[1].x)
          const second_atan_old = 180 / Math.PI * Math.atan2(this.data._touch_img_relative[1].y, this.data._touch_img_relative[1].x)
          // 当前旋转的角度
          const first_deg = first_atan - first_atan_old
          const second_deg = second_atan - second_atan_old
          if (first_deg != 0) {
    ...
    _observe (obj, key, watchFun) {
      ...
      Object.defineProperty(obj, key, {
        ...
        set: (value) => {
          val = value
          watchFun && watchFun(val, this)// todo
        },
        get () {
